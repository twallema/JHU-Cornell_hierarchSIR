...
df["log_rel_wis"] = np.log(df[f"relative_WIS_{baseline}"])
...
with pm.Model() as learning_model:
    ...
    ## asymptote \mu
    mu_intercept = pm.Normal("mu_intercept", -1.0, 1/3)
    ...
    mu = mu_intercept + mu_ED + mu_season[season_idx] + mu_model[model_idx]
...
variables2plot = [
    "sigma", "sigma_ref",
    "mu_intercept", "mu_ED", "mu_season", "mu_model",
    "Delta_model", "Delta_season", "Delta_ED",
    "kappa_model", "kappa_season", "kappa_ED"
    ]
os.makedirs('pyMC_output', exist_ok=True)
for var in variables2plot:
    arviz.plot_trace(trace, var_names=[var]) 
    plt.savefig(f'pyMC_output/trace-{var}.pdf')
    plt.close()
...
print(summary_df)
summary_df.to_csv(f'pyMC_output/coefficients_{baseline}.csv', index=False)
...
for g_vals, idx in df_groups.groupby(group_cols).groups.items():
    ...
    results.append({
        ...
        "ED": g_vals[3],
        "modeled_gm": np.exp(mean_log_wis)
    })
...
empirical_grouped = (
    df.groupby(["model", "season", "training_horizon", "ED"])[f"relative_WIS_{baseline}"]
      .apply(geo_mean)
      .reset_index(name="empirical_gm")
)
...
plot_df.to_csv(f'pyMC_output/2plot_{baseline}.csv', index=False)